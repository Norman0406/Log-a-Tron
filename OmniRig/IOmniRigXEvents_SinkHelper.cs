...
namespace OmniRig
{
    ...
    public sealed class IOmniRigXEvents_SinkHelper : IOmniRigXEvents
    {
        ...
        public override void VisibleChange()
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            if (m_VisibleChangeDelegate != null)
            {
                m_VisibleChangeDelegate();
            }
        }

        public override void RigTypeChange(int P_0)
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            if (m_RigTypeChangeDelegate != null)
            {
                m_RigTypeChangeDelegate(P_0);
            }
        }

        public override void StatusChange(int P_0)
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            if (m_StatusChangeDelegate != null)
            {
                m_StatusChangeDelegate(P_0);
            }
        }

        public override void ParamsChange(int P_0, int P_1)
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            if (m_ParamsChangeDelegate != null)
            {
                m_ParamsChangeDelegate(P_0, P_1);
            }
        }

        public override void CustomReply(int P_0, object P_1, object P_2)
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            if (m_CustomReplyDelegate != null)
            {
                m_CustomReplyDelegate(P_0, P_1, P_2);
            }
        }
        ...
        internal IOmniRigXEvents_SinkHelper()
        {
            //Error decoding local variables: Signature type sequence must have at least one element.
            m_dwCookie = 0;
            m_VisibleChangeDelegate = null;
            m_RigTypeChangeDelegate = null;
            m_StatusChangeDelegate = null;
            m_ParamsChangeDelegate = null;
            m_CustomReplyDelegate = null;
        }
    }
}
